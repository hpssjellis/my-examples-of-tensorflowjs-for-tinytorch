...
void load_images_from_sd() {
  ...
  if(!myRoot) { Serial.println("[ERR] SD /images folder not found"); return; }
  ...
  while(myFolder && myCls < 3) {
    if(myFolder.isDirectory()) {
      ...
      if(myName.startsWith(".") || myName=="header" || myName=="System Volume Information") {
...
bool save_model_header() {
  ...
  fs::File file = SD.open("/header/myModel.h", FILE_WRITE);
  if(!file) {
