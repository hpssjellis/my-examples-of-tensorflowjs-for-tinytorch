/*
 * ESP32-S3 On-Device CNN Training - FOLDER ORGANIZED VERSION
 * Images stored in /images/[label]/
 * Weights stored/loaded from /header/myModel.h
 */
...
bool INITIALIZED_WEIGHTS = false;

// Global flag to track if we found existing progress to load
...
// Helper to parse the .h file text into the buffers
...
void initialize_weights() {
  if (INITIALIZED_WEIGHTS) return;

  // Persistence check: Try to load existing header
  File myHeaderFile = SD.open("/header/myModel.h", FILE_READ);
  if (myHeaderFile) {
      Serial.println("Existing model found. Loading weights from /header/myModel.h...");
      int c1w_sz = USE_GRAYSCALE_MODE ? 36 : 108;
      myParseWeightLine(myHeaderFile, conv1_w, c1w_sz);
      myParseWeightLine(myHeaderFile, conv1_b, 4);
      myParseWeightLine(myHeaderFile, conv2_w, 288);
      myParseWeightLine(myHeaderFile, conv2_b, 8);
      myParseWeightLine(myHeaderFile, dense_w, FLATTENED_SIZE * 3);
      myParseWeightLine(myHeaderFile, dense_b, 3);
      myHeaderFile.close();
  ...
  Serial.println("No existing model found. Initializing with random weights...");
  int c1w_sz = USE_GRAYSCALE_MODE ? 36 : 108;
...
void print_debug() {
  ...
  Serial.println("\nLogits:");
  for(int i=0; i<3; i++) {
    Serial.printf("  Class %d (%s): %.4f\n", i, class_labels[i].c_str(), log(max(dense_output[i], 1e-7f)));
  }
...
bool save_model_header() {
  ...
  for(int i=0; i<3; i++) {
    file.print("  \""); file.print(toCString(class_labels[i])); file.print("\"");
...
void setup() {
  ...
  load_images_from_sd();
  if(!allocate_model_memory()) while(1) delay(1000);
