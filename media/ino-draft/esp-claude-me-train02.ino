/*
 * ESP32-S3 On-Device CNN Training - COMPLETE VERSION
...
void initialize_weights() {
  if (INITIALIZED_WEIGHTS){
    ...
    INITIALIZED_WEIGHTS = true;  // only do this once
...
void load_images_from_sd() {
  Serial.println("\n=== Loading Images ===");
  File root = SD.open("/");
  if(!root) { Serial.println("[ERR] SD root"); return; }
...
// SAVE MODEL TO SD CARD - FIXED VERSION
bool save_model_header() {
  Serial.println("\n=== Saving Model ===");
  
  // Check if header folder exists, create if not
  if(!SD.exists("/header")) {
    if(!SD.mkdir("/header")) {
  ...
  // Delete old file if exists (critical for preventing corruption)
  if(SD.exists("/header/myModel.h")) {
    SD.remove("/header/myModel.h");
    delay(100); // Give SD time to complete deletion
  }
  ...
  // Write header file in chunks to avoid memory issues
  ...
  if(USE_INT8_QUANTIZATION) {
    ...
    for(int i=0; i<c1w_sz; i++) {
      ...
      if(i<c1w_sz-1) file.print(", ");
      if((i+1)%10==0) { file.println(); file.flush(); } // Flush periodically
    }
    file.println(" };");
  }
  file.println();
  ...
  // Dense weights (20184 elements - write in chunks)
  ...
  if(USE_INT8_QUANTIZATION) {
    ...
    for(int i=0; i<FLATTENED_SIZE*3; i++) {
      ...
      if((i+1)%50==0) { file.println(); file.flush(); } // Flush every 50 values
    }
    file.println(" };");
  } else {
...
void setup() {
  ...
  if(training_data.size() < 3) {
    Serial.println("[ERR] Need at least 3 images");
    while(1) { delay(1000); }
  }
