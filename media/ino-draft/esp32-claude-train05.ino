...
void myPrintDebug() {
  ...
  if(myTrainingData.size() > 0) {
      // Test on the first image of the dataset
      ...
      // ... (keep the print_stats lambda and logic)
      
      Serial.println("\nProbabilities:");
      Serial.printf("  [0:%.1f%%, 1:%.1f%%, 2:%.1f%%]\n", myDenseOutput[0]*100, myDenseOutput[1]*100, myDenseOutput[2]*100);
      Serial.printf("  Target Label was: %d\n", myTrainingData[0].label);
  }
  
  Serial.println("===================================");
  Serial.println(">>> TRAINING SESSION FINISHED <<<");
  Serial.println(">>> Press A0 to start a new training session anytime. <<<");
}
...
// LOOP
void loop() {
  // Use a flag so we only print the "Ready" message once
  static bool myReadyMessageShown = false;
  if (!myReadyMessageShown) {
    Serial.println("\nSystem Ready. Press A0 to start training...");
    myReadyMessageShown = true;
  }
  ...
  if(analogRead(A0) > 2000) {
    ...
    myReadyMessageShown = false; // Reset to show message again after 2 seconds
    delay(2000);
  }
  delay(100);
}