...
bool myAllocateModelMemory() {
  ...
  if(!myConv1_w || !myConv2_w || !myDense_w) return false;
...
void myAugmentImage(float* src, float* dst) {
  ...
  for(int i=0; i<sz; i++) dst[i] = myClipValue((src[i]-0.5)*co+0.5+br, 0.0, 1.0);
}
...
void myBackwardConv2() {
  for(int i=0; i<myFlattenedSize; i++) myConv2Grad[i] = myDenseGrad[i]*myDropoutMask[i]*myLeakyReluDeriv(myConv2Output[i]);
...
void myUpdateWeights(int step) {
  myAdamUpdate(myConv1_w, myConv1_w_grad, myConv1_w_m, myConv1_w_v, USE_GRAYSCALE_MODE?36:108, step);
...
void myTrainModel() {
  ...
  float running_loss = 0; int loss_count = 0;
  for(int batch=0; batch<total_batches; batch++) {
    ...
    float batch_loss = 0; int correct = 0;
    for(int i=batch_start; i<batch_end; i++) {
      int idx = indices[i];
      myAugmentImage(myTrainingData[idx].data, myInputBuffer);
      myForwardConv1(); myForwardPool1(); myForwardConv2(); myForwardDropout(true); myForwardDense();
      myBackwardDense(myTrainingData[idx].label); myBackwardConv2(); myBackwardPool1(); myBackwardConv1();
      float loss = 0;
      for(int c=0; c<3; c++) {
        float target = (c == myTrainingData[idx].label) ? 1.0f : 0.0f;
      ...
      if(pred == myTrainingData[idx].label) correct++;
    }
    myUpdateWeights(batch+1);
    running_loss += batch_loss / (batch_end - batch_start); loss_count++;
    if((batch+1) % 10 == 0) {
      Serial.printf("Batch %d/%d - Loss: %.4f, Acc: %.1f%%\n", batch+1, total_batches, running_loss/loss_count, 100.0*correct/(batch_end-batch_start));
...
void myPrintDebug() {
  ...
  auto print_stats = [](const char* name, float* buf, int len) {
    float mn=1e6, mx=-1e6, sum=0; int neg=0, zeros=0;
    for(int i=0; i<len; i++) {
      if(buf[i]<mn) mn=buf[i]; if(buf[i]>mx) mx=buf[i]; sum += buf[i];
      if(buf[i]<0) neg++; if(buf[i]==0) zeros++;
    }
    Serial.printf("  %s: Min=%.2f Max=%.2f Avg=%.2f Neg=%d%% Zeros=%d%%\n", name, mn, mx, sum/len, (neg*100)/len, (zeros*100)/len);
  };
  print_stats("CONV1", myConv1Output, 15376); print_stats("POOL1", myPool1Output, 3844); print_stats("CONV2", myConv2Output, myFlattenedSize);
...
bool mySaveModelHeader() {
  ...
  SD.remove("/header/myModel.h"); delay(100);
  File file = SD.open("/header/myModel.h", FILE_WRITE);
  if(!file) return false;
  file.println("// Auto-generated\n#ifndef MY_MODEL_H\n#define MY_MODEL_H\n");
  file.println("const char* myClassLabels[3] = {");
  for(int i=0; i<3; i++) { file.print("  \""); file.print(myToCString(myClassLabels[i])); file.println(i<2?"\",":"\""); }
  file.println("};");
  int c1w_sz = USE_GRAYSCALE_MODE ? 36 : 108;
  file.print("const float myConv1_w[] = { ");
  for(int i=0; i<c1w_sz; i++) { file.print(myConv1_w[i],6); file.print(i<c1w_sz-1?"f, ":"f };\n"); }
  file.print("const float myConv1_b[] = { ");
  for(int i=0; i<4; i++) { file.print(myConv1_b[i],6); file.print(i<3?"f, ":"f };\n"); }
  file.print("const float myConv2_w[] = { ");
  for(int i=0; i<288; i++) { file.print(myConv2_w[i],6); file.print(i<287?"f, ":"f };\n"); }
  file.print("const float myConv2_b[] = { ");
  for(int i=0; i<8; i++) { file.print(myConv2_b[i],6); file.print(i<7?"f, ":"f };\n"); }
  file.print("const float myOutput_w[] = { ");
  for(int i=0; i<myFlattenedSize*3; i++) { file.print(myDense_w[i],6); file.print(i<myFlattenedSize*3-1?"f, ":"f };\n"); }
  file.print("const float myOutput_b[] = { ");
  for(int i=0; i<3; i++) { file.print(myDense_b[i],6); file.print(i<2?"f, ":"f };\n"); }
  file.println("\n#endif");
  file.close(); return true;
}

void setup() {
  Serial.begin(115200); pinMode(A0, INPUT); pinMode(LED_BUILTIN, OUTPUT); delay(2000);
  if(!SD.begin(21)) { Serial.println("SD card failed"); while(1); }
  myLoadImagesFromSd();
  if(!myAllocateModelMemory()) while(1);
  Serial.println("\nSystem Ready. Press A0 to start training...");
}
...
void loop() {
  if(analogRead(A0) > 2000) {
    ...
    if(!SD.begin(21)) {
        ...
        myInitializeWeights();
        myTrainModel();
        if(mySaveModelHeader()) Serial.println("\n✓ Model saved!"); else Serial.println("\n✗ Save failed!");
        myPrintDebug();
    }
