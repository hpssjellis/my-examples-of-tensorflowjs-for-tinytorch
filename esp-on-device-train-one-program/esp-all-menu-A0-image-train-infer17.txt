...
// Load and preprocess image from SD card
bool myLoadImageFromFile(const char* path, float* buf) {
  ...
  // Resize 240x240 to 64x64 and normalize
  for(int y=0; y<64; y++) {
    for(int x=0; x<64; x++) {
...
void myForwardPass(float* input, float* logits) {
  ...
  for(int c=0; c<NUM_CLASSES; c++) {
    double sum = 0, comp = 0; // Kahan summation for numerical stability
...
/* * ======================================================
 * CNN BACKPROPAGATION - Gradient Calculation
 * ======================================================
 */
...
void myActionCollect(int classIdx) {
  ...
  // Count existing images in each class
  int counts[3] = {0, 0, 0};
  for(int i=0; i<3; i++) {
  ...
  do {
    ...
    u8g2.setCursor(0, 22);
    u8g2.print("Imgs: ");
    u8g2.print(counts[0]); u8g2.print(",");
    u8g2.print(counts[1]); u8g2.print(",");
    u8g2.print(counts[2]);
    u8g2.drawStr(0, 34, "TAP:Snap HOLD:Exit");
  } while (u8g2.nextPage());
...
void myActionTrain() {
  ...
  while (true) {
    ...
    Serial.printf("Training with %d images (%d batches per epoch)\n", total, batchesPerEpoch);
    Serial.println("\n--- Starting Training ---");
    ...
    for(int batch=0; batch<totalBatches; batch++) {
      ...
      for(int i=batchStart; i<batchEnd; i++) {
        ...
        // Load image
        if(!myLoadImageFromFile(img.path.c_str(), myInputBuffer)) {
          continue;
        }
  ...
  while (true) {
    ...
    if (fb) {
      ...
      if (rgb && fmt2rgb888(fb->buf, fb->len, PIXFORMAT_JPEG, rgb)) {
        ...
        // myDense_output now contains softmax probabilities from forward pass
        int pred = 0;
        for(int i=1; i<3; i++) if(myDense_output[i] > myDense_output[pred]) pred = i;
        ...
        do {
          ...
          u8g2.setCursor(0, 38);
          u8g2.print((int)(myDense_output[pred] * 100)); u8g2.print("% Conf");
        } while (u8g2.nextPage());
        
        Serial.printf("Prediction: %s (%.1f%%), All: (%.0f%%, %.0f%%, %.0f%%)\n", 
